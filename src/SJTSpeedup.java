...
public class SJTSpeedup extends SJTOrderPermutation implements Runnable {
	private List<Integer> step;
	private CountDownLatch latch; 
	
	public SJTSpeedup(int per_size, int index, int num, CountDownLatch latch) {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < threads; i++) {
			t[i] = new Thread(new SJTSpeedup(per_size, i, threads, latch));
			t[i].start();
		}
